...
public class ArithmeticExpression implements Expression {
    ...
    @Override
    public Value evaluate(ISymbolTable symbolTable, IHeap heap) throws ArithmeticRelationalExpressionException, DivisionByZeroException {
        ...
        switch (operator) {
            case "+" -> {
                return new IntegerValue(int_left + int_right);
            }
            case "-" -> {
                return new IntegerValue(int_left - int_right);
            }
            ...
            case "/" -> {
                if (int_right == 0) throw new DivisionByZeroException();
                else return new IntegerValue(int_left / int_right);
            }
